...
リモートアクセスと本番利用のため、VPSまたはDockerにJet Routerをデプロイ。
...
| `DATA_DIR` | `~/.jet-router` | データベースとデータの保存パス |
...
`/etc/nginx/sites-available/jet-router` を作成:
...
# リバースプロキシを使用しない場合、Jet Routerポートを許可
