...
Jet Routerに関する一般的な質問。

---

## Jet Routerとは?

**Jet Routerは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、Jet Router自体は100%無料でオープンソースです。**
...
**Jet Routerはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## Jet Routerをセルフホストできますか?

**はい! Jet Routerは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.jet-router`)
...
**はい、Jet Routerはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.jet-router`(またはカスタム`DATA_DIR`)にローカル保存
- Jet Routerサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
...
**破壊的変更:**
- [CHANGELOG.md](https://github.com/Jethin10/Jet-Router/blob/main/CHANGELOG.md)を確認
- メジャー更新前に`~/.jet-router`をバックアップ
- メジャーバージョンの移行ガイドを確認
...
詳細は[CONTRIBUTING.md](https://github.com/Jethin10/Jet-Router/blob/main/CONTRIBUTING.md)を参照。

---

## さらにヘルプが必要?

- **ドキュメント:** [Jet Router docs](https://router.example.com/docs)
