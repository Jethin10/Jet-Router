...
Triển khai Jet Router trên VPS hoặc Docker để truy cập từ xa và dùng trong production.
...
| `DATA_DIR` | `~/.jet-router` | Đường dẫn lưu database và data |
...
Tạo `/etc/nginx/sites-available/jet-router`:
...
# If NOT using reverse proxy, allow Jet Router ports
...
```bash
# Check if Jet Router is running
pm2 status
