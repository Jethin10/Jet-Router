...
Tích hợp Jet Router với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của Jet Router.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- Jet Router đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint Jet Router:

**Cho Jet Router cục bộ:**
```
http://localhost:20128/v1
```

**Cho Jet Router cloud:**
```
https://router.example.com
```

**Các bước:**
1. Trong field **Base URL**, nhập endpoint Jet Router
2. Đảm bảo bao gồm `/v1` ở cuối

### 4. Thêm API Key

1. Trong field **API Key**, nhập API key Jet Router của bạn
2. Bạn có thể tìm API key trong Jet Router dashboard tại **Settings → API Keys**
3. Key bắt đầu bằng `sk-jet-router-`

### 5. Chọn Model

1. Trong dropdown **Model**, bạn có thể:
   - Chọn từ model có sẵn (nếu Cline auto-detect)
   - Nhập tên model thủ công từ cấu hình Jet Router
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng Jet Router để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua Jet Router
...
3. Nhận giải thích AI qua Jet Router

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng Jet Router để hiểu context và thực hiện thay đổi
...
1. Xác minh Jet Router đang chạy: `curl http://localhost:20128/health`
...
2. Xác minh Jet Router instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs Jet Router để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng Jet Router cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: `https://router.example.com`
2. Đảm bảo bạn đã cấu hình API key trong Jet Router cloud dashboard
...
## Tích hợp với Tính năng Jet Router

### Định tuyến Model

Jet Router tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, Jet Router tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua Jet Router dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
