...
Despliega Jet Router en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.jet-router` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/jet-router
sudo chown $USER:$USER /var/lib/jet-router
```
...
Crea `/etc/nginx/sites-available/jet-router`:
...
sudo ln -s /etc/nginx/sites-available/jet-router /etc/nginx/sites-enabled/
...
# Si NO usas proxy reverso, permite los puertos de Jet Router
...
tar -czf jet-router-backup-$(date +%Y%m%d).tar.gz /var/lib/jet-router

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/jet-router-$(date +\%Y\%m\%d).tar.gz /var/lib/jet-router
```
...
```bash
# Verificar si Jet Router está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/jet-router
chmod 755 /var/lib/jet-router
```
