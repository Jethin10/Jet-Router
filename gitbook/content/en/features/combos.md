...
Combos are **custom fallback chains** that you create in the dashboard. Instead of using a single model, you define a sequence of models that Jet Router tries in order.
...
Jet Router automatically tries each model in sequence until one succeeds.
...
When limit reached, Jet Router skips paid models and uses free tier only.
