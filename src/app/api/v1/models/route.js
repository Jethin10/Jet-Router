...
const LIVE_MODEL_RESOLVERS = {
  agentrouter: async (conn) => {
    const url = AI_PROVIDERS.agentrouter?.modelsFetcher?.url;
    if (!url || !conn?.apiKey) return null;
...
export async function buildModelsList(kindFilter, options = {}) {
  ...
  if (connections.length === 0) {
    ...
    for (const [providerId, providerConnections] of activeConnectionsByProvider.entries()) {
      ...
      const accountCatalogs = await Promise.all(providerConnections.map(async (connection) => {
        ...
        if (!skipDynamicFetch && liveResolver) {
          try {
            const live = await liveResolver(connection);
            if (live?.models?.length) return live;
          } catch (err) {
            console.log(`Live model fetch failed for ${providerId}: ${err?.message || err}`);
          }
        }

        if (!skipDynamicFetch && isCompatibleProvider) {
          const compatibleIds = await fetchCompatibleModelIds(connection);
          if (compatibleIds.length) {
            return { models: compatibleIds.map((id) => ({ id })) };
          }
        }
      ...
      // A provider can have accounts with different entitlements. Merge their
      // catalogs so clients can select any model reachable through the pool.
      const rawModels = accountCatalogs.flatMap((catalog) => catalog.models || []);
      let rawModelIds = rawModels.map((model) => model.id);
      const liveModelNameById = new Map(
        rawModels
          .filter((model) => model?.id && (model.name || model.display_name))
          .map((model) => [model.id, model.name || model.display_name])
      );
      liveModelKindById = new Map(
        rawModels
          .filter((model) => model?.id)
          .map((model) => [model.id, modelKind(model)])
      );
      liveCapabilitiesById = new Map(
        rawModels
          .filter((model) => model?.id && model.capabilities)
          .map((model) => [model.id, model.capabilities])
      );

      // Keep the old all-static fallback for providers with no usable account
      // catalog, including temporary /models outages.
      ...
      for (const modelId of mergedModelIds) {
        ...
        const model = {
          ...
          name: liveModelNameById.get(modelId)
            || providerModels.find((entry) => entry.id === modelId)?.name
            || modelId,
        };
...
export async function authorizeModelsRequest(request) {
  ...
  const authenticationError = (message) => Response.json(
    wantsAnthropicModelsFormat(request)
      ? { type: "error", error: { type: "authentication_error", message } }
      : { error: { message, type: "authentication_error", code: "invalid_api_key" } },
    { status: 401, headers: { "Access-Control-Allow-Origin": "*" } },
  );
  if (!apiKey) {
    return authenticationError("Missing API key");
  }
...
export function formatModelsResponse(request, models) {
  if (wantsAnthropicModelsFormat(request)) {
