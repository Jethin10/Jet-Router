...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-jet-router.env");
};
...
const hasJetRouterConfig = (config) => {
  ...
  if (providers["jet-router"]) return true;
...
export async function GET() {
  ...
  const hasJetRouter = hasJetRouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_JET_ROUTER_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile jet-router",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["jet-router"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_JET_ROUTER_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jet-router configuration removed from jcode",
    });
  } catch (error) {
