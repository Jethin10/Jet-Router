...
// Check if settings has Jet Router config
const hasJetRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["jet-router"];
};

// Read per-agent models.json and return current model id (without "jet-router/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["jet-router"]?.models;
...
// POST - Update Jet Router settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `jet-router/${model}`;

    // Remove all old jet-router/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("jet-router/"))
    ...
    // Add fresh jet-router models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`jet-router/${m}`] = {};
    });

    // Remove old jet-router model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("jet-router/")) {
    ...
    // Update models.providers.jet-router with all models
    settings.models.providers["jet-router"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `jet-router/${agentModel}` };
        return agent;
      });
...
export async function DELETE() {
  try {
    ...
    // Remove Jet Router from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["jet-router"];
    ...
    // Remove jet-router models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("jet-router/"));
    ...
    // Reset agents.defaults.model.primary if it uses jet-router
    if (settings.agents?.defaults?.model?.primary?.startsWith("jet-router/")) {
