...
const hasJetRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "Jet Router");
};

const getJetRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "Jet Router") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getJetRouterEntry(config);
...
// POST - Apply Jet Router config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing Jet Router entry or append
    const idx = config.findIndex((e) => e.name === "Jet Router");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Jet Router entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Jet Router");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Jet Router removed from Copilot config",
    });
  } catch (error) {
