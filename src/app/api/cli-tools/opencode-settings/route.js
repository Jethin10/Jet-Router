...
const hasJetRouterConfig = (config) => {
  ...
  return !!config.provider["jet-router"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["jet-router"];
    ...
    return NextResponse.json({
      ...
      configPath: getConfigPath(),
        opencode: {
          ...
          activeModel: config?.model?.startsWith("jet-router/") ? config.model.replace(/^jet-router\//, "") : null,
...
// POST - Apply Jet Router as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_jet-router";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing jet-router provider entry and its models
    const existingProvider = config.provider["jet-router"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["jet-router"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `jet-router/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `jet-router/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("jet-router/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove Jet Router provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["jet-router"]?.models) {
      delete config.provider["jet-router"].models[modelToRemove];

      // If no models left, remove the provider
      if (Object.keys(config.provider["jet-router"].models).length === 0) {
        delete config.provider["jet-router"];
        if (config.model?.startsWith("jet-router/")) delete config.model;
      } else if (config.model === `jet-router/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["jet-router"].models);
        config.model = `jet-router/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire jet-router provider
      if (config.provider) delete config.provider["jet-router"];
      if (config.model?.startsWith("jet-router/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("jet-router/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "Jet Router settings removed from OpenCode",
    });
  } catch (error) {
