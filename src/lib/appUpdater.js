...
// Collect PIDs of all jet-router-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from jet-router install dir or wrapper cli.js
        const isAppProcess = lower.includes("jet-router") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("jet-router") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "jet-router");
  }
  return path.join(os.homedir(), ".jet-router");
}
...
// Resolve npx/jet-router binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx jet-router` — works regardless of global bin path changes after npm i -g
