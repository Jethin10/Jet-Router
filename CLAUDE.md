...
- The **dashboard + gateway** (root `package.json`, `jet-router-app`) — the Next.js server that does the actual routing.
- The **CLI launcher** (`cli/`, published to npm as `jet-router`) — a separate package that installs/starts the server and manages the tray. It has its own `package.json`, version, and build.
...
- DB file location resolves via `src/lib/db/paths.js` (`DATA_DIR`, else `~/.jet-router/`).
- Usage/logs (`src/lib/usageDb.js`, `usage.json` + `log.txt`) still live under `~/.jet-router` and do **not** follow `DATA_DIR`.
