# Jet Router Architecture

_Last updated: 2026-02-06_

## Executive Summary

Jet Router is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[Jet Router Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.jet-router/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.jet-router/usage.json`, `~/.jet-router/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.jet-router/db.json`)
- usage stats: `~/.jet-router/usage.json`
- request log lines: `~/.jet-router/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Jet Router Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.jet-router` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/jet-router && npm run build`
- Build Docker image: `cd /root/dev/jet-router && docker build -t jet-router .`
