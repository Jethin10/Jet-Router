---
name: jet-router-chat
description: Chat / code generation via Jet Router using OpenAI /v1/chat/completions or Anthropic /v1/messages format with streaming + auto-fallback combos. Use when the user wants to ask an LLM, generate code, summarize text, or run prompts through Jet Router.
---
...
- `POST $JET_ROUTER_URL/v1/chat/completions` — OpenAI format
- `POST $JET_ROUTER_URL/v1/messages` — Anthropic format

## Discover

```bash
curl $JET_ROUTER_URL/v1/models | jq '.data[].id'
# Per-model metadata (contextWindow, params)
curl "$JET_ROUTER_URL/v1/models/info?id=openai/gpt-4o"
```
...
```bash
curl -X POST $JET_ROUTER_URL/v1/chat/completions \
...
const client = new OpenAI({ baseURL: `${process.env.JET_ROUTER_URL}/v1`, apiKey: process.env.JET_ROUTER_KEY });
...
```bash
curl -X POST $JET_ROUTER_URL/v1/messages \
  -H "Authorization: Bearer $JET_ROUTER_KEY" \
