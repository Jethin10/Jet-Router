---
name: jet-router-tts
description: Text-to-speech via Jet Router /v1/audio/speech using OpenAI / ElevenLabs / Deepgram / Edge TTS / Google TTS / Hyperbolic / Inworld voices. Use when the user wants to convert text to speech, generate audio, voiceover, narrate, or read text aloud.
---

# Jet Router — Text-to-Speech
...
```bash
# 1) List models
curl $JET_ROUTER_URL/v1/models/tts | jq '.data[].id'
# 2) Per-model metadata (params, voicesUrl if voice-by-id)
curl "$JET_ROUTER_URL/v1/models/info?id=el/eleven_multilingual_v2"
# 3) List voices (elevenlabs, edge-tts, deepgram, inworld, local-device). Optional ?lang=vi
curl "$JET_ROUTER_URL/v1/audio/voices?provider=edge-tts&lang=vi" | jq '.data[].model'
```
...
`POST $JET_ROUTER_URL/v1/audio/speech`
...
```bash
curl -X POST "$JET_ROUTER_URL/v1/audio/speech" \
...
const r = await fetch(`${process.env.JET_ROUTER_URL}/v1/audio/speech`, {
  method: "POST",
