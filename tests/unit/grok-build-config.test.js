...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("jet-router");
    ...
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "jet-router-general-purpose",
      explore: "jet-router-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.jet-router\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.jet-router-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.jet-router-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# jet-router-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.jet-router-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("jet-router-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.jet-router-");
    expect(reset).not.toContain("jet-router-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("jet-router-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("jet-router-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("jet-router-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("jet-router-explore");
    expect(getGrokSubagentSlot("plan")).toBe("jet-router-plan");
