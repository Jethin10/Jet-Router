...
async function buildCodexHeader() {
  ...
  const { installed, hasJetRouter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasJetRouter, settings } = result.data;
  ...
  // Extract Jet Router custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:Jet Router-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract Jet Router provider config
  const provider = settings?.models?.providers?.["jet-router"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("jet-router/") ? primary.replace("jet-router/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasJetRouter, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use Jet Router\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
