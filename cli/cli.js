...
// Subcommands (`jet-router xai video …`) run against an already-running gateway
...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.jet-router/runtime
...
// Identifiers for killAllAppProcesses - only kill jet-router specifically
const PROCESS_IDENTIFIERS = [
  'jet-router'  // Only package name - avoid killing other apps
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "jet-router")
    : path.join(os.homedir(), ".jet-router");
}
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running jet-router/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "jet-router" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("jet-router") && (cmd.includes("cli.js") || cmd.includes("\\jet-router") || cmd.includes("/jet-router")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running jet-router/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "jet-router".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("jet-router") && (cmd.includes("cli.js") || cmd.includes("/jet-router")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
function startServer(updatePromise) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 Jet Router is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 Jet Router is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "jet-router", "db.json") : path.join(".jet-router", "db.json"));
